...
def predict(req: PredictRequest) -> PredictResponse:
    ...
    # Parent span is made *current* so embed-text/vector-search/generate-tokens
    # nest under it as one trace (POST /predict + 3 child spans) instead of
    # exporting as separate single-span root traces.
    try:
        with tracer.start_as_current_span("predict") as span:
            ...
            if req.fail:
                ...
                # Mark the span ERROR so the collector's keep-errors tail-sampling
                # policy retains it 100% (otherwise it falls to the 1% probabilistic
                # policy and the trace you most need gets dropped).
